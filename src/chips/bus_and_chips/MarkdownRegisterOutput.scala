...
{
  def generateMarkdowns(chip : Chip, headerPath : String) : Unit =
  {
    if(chip.isHard) 
    {
      generateHardMarkdowns(chip.asInstanceOf[HardChip], headerPath)
    }
    else
    {
      generateCustomMarkdowns(chip.asInstanceOf[CustomChip], headerPath)
    }
  }
  ...
  private def generateHardMarkdowns(chip : HardChip, markdownPath : String) : Unit =
  {
    def repeatChar(c: Char, n: Int): String = c.toString * n
    def makeField(s: String, w: Int) : String = {
      if(s.length() >= w) s"| ${s.substring(0, w-2)} " 
      else s"|$s${repeatChar(' ', w - s.length)}"
    }

    def makeHeader(fields : Vector[(String, Int)]) : String = {
    ...
    chip.registerBanks.foreach( rb => {
      var str = ""
      val filename = markdownPath + rb.name + ".md"
      ...
      rb.registers.foreach( r => {
        ...
        val shortName = r.substring(r.indexOf("_")+1)
//        s ++= makeField(f"[$r](#$r)", regWidth)
  ...
  private def generateCustomMarkdowns(chip : CustomChip, path : String) : Unit =
  {
